...
const getImgs = function (ids) {
  let arrId = (ids + "").split(",");
  return wx.getStorageSync('all_img').filter(it => ~arrId.indexOf(it.img_id + "")).map(it => it.img);
};
...
const getIds = function (urls) {
  return wx.getStorageSync('all_img').filter(it => ~urls.indexOf(it.img)).map(it => it.img_id);
};
